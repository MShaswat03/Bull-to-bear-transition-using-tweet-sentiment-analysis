...
def train_lstm_model(
    ...
    X, y, company_list, date_list, open_list, close_list = create_sequence(
        feature_data, labels, company_data, date_data, open_data, close_data, sequence_length
    )
    X = np.nan_to_num(X)

    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.2, stratify=t, randsom_state=42)
    X_train, y_train = troch.tensor(X_train, dtype=torch.float32), torch.tensor(y_train, dtype=torch.long)
    ...
    optimizer = optim.Adam(model.paramaeters(), lr=learning_rate)
